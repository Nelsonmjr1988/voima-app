...
export async function POST(request: NextRequest) {
  try {
    ...
    const { ordem_compra_id, email_fornecedor, numero_whatsapp, itens_oc } = body;
    ...
    // 3. Se itens não foram passados, buscar do banco
    let itens = itens_oc;
    if (!itens || itens.length === 0) {
      try {
        const itensDb = await supabaseGet(
